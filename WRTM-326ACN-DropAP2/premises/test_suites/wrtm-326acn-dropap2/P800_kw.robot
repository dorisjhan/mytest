...
login ont
    ...
    ${r}  run keyword and return status    Wait Until Keyword Succeeds    3x    2s    cpe click    ${browser}    xpath=//*[@id="btn-control"]
    ...
    page should contain text    ${browser}    Status
...
wait action finish
    ...
    #${block_exist}    run keyword and return status    wait_until_element_is_visible    ${browser}    ${elem_locator}
    #return from keyword if  ${block_exist}==False
    # wait until element not visible
    set_implicit_wait_time    ${browser}    ${origin_wait_time}
    wait_until_element_is_not_visible    ${browser}    ${elem_locator}
