...
${g_844_wifi_client_ip1}    192.168.1.133
${g_844_wifi_client_ip2}    192.168.10.133
${g_844_wifi_client_ip3}    192.168.12.133
${dut_wifi_ip_start3}    192.168.12.2
${dut_wifi_ip_end3}    192.168.12.254
${g_844_wifi_client_ip4}    192.168.14.133
${dut_wifi_ip_start4}    192.168.14.2
${dut_wifi_ip_end4}    192.168.14.254
${dut_gw1}    192.168.1.1
${dut_gw2}    192.168.10.1
${dut_gw3}    192.168.12.1
${dut_gw4}    192.168.14.1
...
*** Test Cases ***
tc_RG_WiFi5G_GUI_Checking_Pck_SentReceived
    [Documentation]   tc_RG_WiFi5G_GUI_Checking_Pck_SentReceived
    ...    #Topology #Wifi_client.....CPE-----E5.....WanHost
    ...    1 Go to Wan service page configure ipv4 IPoE settings
    ...    2.Go to 5G Wi-Fi configure SSID name & Security Key & Network Domain
    ...
    ...    6.Duplicate step2~ step5 by configuring SSID name & Security Key & Network Domain
    [Tags]    @TCID=STP_DD-TC-10920    @globalid= 1526109    @DUT=844FB    @DUT=844F    @AUTHOR=Gemtek_Hans_Sun
    [Timeout]
    #Login Web GUI
    ...
    #Go to Wireless 5G GHz Control Page
    ...
    input_text    web    xpath=//input[@id='id_ssid_name']    ${g_844fb_5g_ssid_name}
    Wait Until Keyword Succeeds    5x    3s    cpe click    web    xpath=//button[contains(., 'Apply')]
    ...
    Login Linux Wifi Client to Connect to DUT in Different 5G SSID    ${g_844fb_5g_ssid_name}    ${g_844_wifi_client_ip1}    ${dut_gw1}
    #Use Wifi Client send traffic to WanHost
    Wait Until Keyword Succeeds    5x    3s    Send Traffic from Linux Wifi Client to WanHost    ${g_844fb_5g_ssid_name}
    ...
    input_text    web    xpath=//input[@id='pskKeyInput']    ${Custom_Security_Key}
    Wait Until Keyword Succeeds    5x    3s    cpe click    web    xpath=//button[contains(., 'Apply')]
    #Linux Wifi Client to Connect to DUT by using Second SSID & 192.168.10.1 network domain
    Login Linux Wifi Client to Connect to DUT in Different 5G SSID    ${Second_SSID}    ${g_844_wifi_client_ip2}    ${dut_gw2}
    #Use Wifi Client send traffic to WanHost
...
#    Wait Until Keyword Succeeds    5x    3s    select_from_list_by_value    web    id=id_ssid    2
#    Wait Until Keyword Succeeds    5x    3s    cpe click    web    xpath=//button[contains(., 'Apply')]
    #Linux Wifi Client to Connect to DUT by using Third SSID & 192.168.12.1 network domain
#    Login Linux Wifi Client to Connect to DUT in Different 5G SSID    ${Third_SSID}    ${g_844_wifi_client_ip3}    ${dut_gw3}
    #Use Wifi Client send traffic to WanHost
#    Wait Until Keyword Succeeds    5x    3s    Send Traffic from Linux Wifi Client to WanHost
    ...
    input_text    web    xpath=//input[@id='pskKeyInput']    ${Custom_Security_Key}
    Wait Until Keyword Succeeds    5x    3s    cpe click    web    xpath=//button[contains(., 'Apply')]
    #Linux Wifi Client to Connect to DUT by using Fourth SSID & 192.168.14.1 network domain
    Login Linux Wifi Client to Connect to DUT in Different 5G SSID    ${Fourth_SSID}    ${g_844_wifi_client_ip4}    ${dut_gw4}
    #Use Wifi Client send traffic to WanHost
    Wait Until Keyword Succeeds    5x    3s    Send Traffic from Linux Wifi Client to WanHost    ${Fourth_SSID}


*** Keywords ***
Login Linux wifi client to connect to DUT in different 5G SSID
    [Arguments]    ${SSID}    ${wifi_client_ip}    ${dut_gw}
    [Documentation]    Linux wifi client to connect to DUT
    [Tags]    @AUTHOR=Gemtek_Hans_Sun

    #Wait for security key is changed, so we sleep here.
    sleep    5s
