...
${sleep_seconds}    120
*** Test Cases ***
tc_RG_2.4GWIFI_WPS_Checking_WPS_Status
    [Documentation]    tc_RG_2.4GWIFI_WPS_Checking_WPS_Status
    ...
    ...   4. Check linux wifi client have shown [WPS-PBC] status to connect to DUT
    [Tags]    @TCID=STP_DD-TC-10890    @globalid=1526059    @DUT=844FB    @DUT=844F    @AUTHOR=Gemtek_Hans_Sun
    ...
    #when GUI show SSID is delayed ,we need to sleep here
    sleep    3s
    ...
    #Check WPS PBC status is exist and wifi client ping DUT is suceessful
    Wait Until Keyword Succeeds    10x    5s    Check WPS PBC    wifi_client    ${g_844fb_2.4g_ssid_name}
...
Check WPS PBC
    ...
    [Documentation]    To check if wifi SSID WPS PBC is exist
    [Tags]    @AUTHOR=Gemtek_Hans_Sun

    #We need to verify wpa_cli wpc_pbc command is completed, so we sleep at here.
    sleep    3s
    cli    wifi_client    sudo -S wpa_cli scan
    sleep    3s
    ${result} =   cli    wifi_client    sudo -S wpa_cli scan_results | grep ${SSID}
    log    ${result}
    Should Contain    ${result}    [WPS-PBC]
