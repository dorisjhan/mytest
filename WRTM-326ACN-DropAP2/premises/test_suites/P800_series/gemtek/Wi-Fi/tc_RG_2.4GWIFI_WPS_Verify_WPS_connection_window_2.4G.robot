...
${sleep_seconds}    120
*** Test Cases ***
tc_RG_2.4GWIFI_WPS_Verify_WPS_connection_window_2.4G
    [Documentation]    tc_RG_2.4GWIFI_WPS_Verify_WPS_connection_window_2.4G
    ...
    ...   4. Repeat step1~step3, Linux wifi client to connect to DUT 2.4 ssid in 30 & 60 & 90 seconds
    [Tags]    @TCID=STP_DD-TC-10889    @globalid=1526058    @DUT=844FB    @DUT=844F    @AUTHOR=Gemtek_Hans_Sun
    ...
    #Linux wifi client to connect to DUT 2.4 ssid in different moment
    Verify WPS PBC Funtion In Different Seconds    ${number_of_times}    ${wait_seconds}    ${g_844fb_2.4g_ssid_name}
...
Verify WPS PBC Funtion In Different Seconds
    [Arguments]     ${times}    ${seconds}    ${SSID}
    [Documentation]    when wait 30 or 60 or 90 seconds to retry clicking WPS button
    [Tags]    @AUTHOR=Gemtek_Hans_Sun

    : FOR    ${INDEX}    IN RANGE    1    ${times}
    \    Wait Until Keyword Succeeds    5x    3s    cpe click    web    xpath=//button[@id='connect_btn']
    \    page should contain text     web    120 seconds left.
    \    ${sleep_time} =   Evaluate    ${INDEX} * ${seconds}
    \    log    ${sleep_time}
    \    sleep    ${sleep_time}
    ...
    \    ${result_mac} =    Wait Until Keyword Succeeds    10x    5s    Get Wifi Mac From SSID Using WPA Supplicant    wifi_client    ${SSID}
    \    log    ${result_mac}
    \    cli    wifi_client    echo 'vagrant' | sudo -S wpa_cli wps_pbc ${result_mac}
    \    Wait Until Keyword Succeeds    5x    5s    cpe click    web    xpath=//button[contains(., 'Ok')]
    \    ${result} =    Wait Until Keyword Succeeds    10x    5s    Is WIFI Interfafe Up    wifi_client    ${SSID}
    \    wait until keyword succeeds     3x   10 sec     ping    ${dut_gw}
    #Anyway, after WPS connected, we sleep 120s to avoid DUT still try to connect possibility.
    \    sleep    120s
