...
Suite Teardown    Run keywords    Restore The Latest Image

*** Variables ***
${Downgrate_image}     /home/vagrant/CALIX_844F_10_14.bin    #Used the previous Calix image.
${Upgrate_image}     /home/vagrant/CALIX_844F_12.1.0.92.bin    #Used the latest Calix image.
${Upload_waiting_time}    150
${Bonding_line1}    line3
${Bonding_line2}    line4

*** Test Cases ***
tc_Monitor_GFAST_link_status_downgrade_844F
    [Documentation]    tc_Monitor_GFAST_link_status_downgrade_844F
    ...    1.Login DUT upgrade image web page
    ...    2.Choose the previous Calix image, and execute downgrate firmware
    ...    3.Use e5 to check GFAST bonding line is down or up
    ...    4.Check DUT web page of GFAST status
    ...    5.Restore the latest image
    [Tags]    @TCID=STP_DD-TC-11690    @globalid=1662615    @DUT=844FB    @DUT=844F    @AUTHOR=Gemtek_Hans_Sun
    [Timeout]
    #Login Web GUI
    ...
    choose file   web    id=fileObjectID    ${Downgrate_image}
    unselect_frame    web
...
Enter Cli Mode
    [Arguments]
    [Documentation]    Enter to cli mode
...
Restore The Latest Image
    [Arguments]
    [Documentation]    DUT restore The Latest Image
    [Tags]    @AUTHOR=Gemtek_Hans_Sun
    ...
    execute javascript    web    document.getElementById("fileUploadFrameObjId").style.display='block'
    select_frame    web    id=fileUploadFrameObjId
    choose file   web    id=fileObjectID    ${Upgrate_image}
    unselect_frame    web
    cpe click    web   id=upgrade_button
    Wait Until Element Is Not Visible    web    xpath=//button[contains(., 'Upgrade')]    ${Upload_waiting_time}    #After upload and reboot finish, web page upgrade button will disappear.
