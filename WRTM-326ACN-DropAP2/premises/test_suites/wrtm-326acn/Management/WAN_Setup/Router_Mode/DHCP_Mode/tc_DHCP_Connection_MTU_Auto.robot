...
*** Test Cases ***
tc_DHCP_Connection_MTU_Auto
    [Documentation]    Ping greater than 1480 bytes packet from LAN to WAN side, check WAN side packet size and occur segment.
    [Tags]    @tcid=WRTM-326ACN-8    @DUT=wrtm-326acn     @AUTHOR=Gemtek_Thomas_Chen
    [Timeout]


    Configure DUT with WAN Internet connection type to be DHCP mode, and MTU Auto mode.
    ...
    Enable the Sniffer to capture Fragmented Packets after ping [1480 - 28] bytes packet from LAN to WAN side PC and Verify that packets of Fragmented IP Protocol which sent from DUT/Server both sides can be captured.
