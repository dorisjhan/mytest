...
${ppp_password}    test
${PPTP_TYPE_FULL}    full
${PPTP_START_WHEN_BOOT}    True
${PPTP_FORCE_ENCRYPTION}    False

*** Test Cases ***
tc_PPTP_Start_on_Boot_Enabled
    ...
    Configure PPTP including server ip, username, password, Select Start on Boot on and save the settings.
    Press "Click" to start the service, then status change "Connected"
    Reload DUT
    Check PPTP Status Should be "Connected"
...
Configure PPTP including server ip, username, password, Select Start on Boot on and save the settings.
    ...
    Config PPTP_L2TP    web    pptp    ${DEVICES.cisco.gateway}    ${ppp_username}    ${ppp_password}    ${PPTP_TYPE_FULL}    ${PPTP_START_WHEN_BOOT}    ${PPTP_FORCE_ENCRYPTION}
...
From Console, Check the PPTP status. Check traffic from lan to vpn internal lan
    ...
    ${dut_wan_ip} =   Wait Until Keyword Succeeds    10x    5s     Get IP Value From Device SSH Connection    dut1_ssh     ${DEVICES.rf1.pptp_interface}
