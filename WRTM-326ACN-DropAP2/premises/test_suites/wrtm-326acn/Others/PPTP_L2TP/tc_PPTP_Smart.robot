...
${ppp_password}    test
${PPTP_TYPE_FULL}    full
${PPTP_TYPE_SMART}    smart
${PPTP_START_WHEN_BOOT}    False
${PPTP_FORCE_ENCRYPTION}    False

*** Test Cases ***
tc_PPTP_Smart
    [Documentation]    Configure PPTP and check status
    [Tags]    @tcid=WRTM-326ACN-155    @DUT=wrtm-326acn     @AUTHOR=Gemtek_Thomas_Chen
    [Timeout]

    Reset DUT and Config DHCP WAN
    Configure PPTP including server ip, username, password, Select "Full" mode and save the settings.
    Press "Click" to start the service, then status change "Connected"
    From Console, Check the PPTP status. Check traffic from lan to vpn internal lan
...
Configure PPTP including server ip, username, password, Select "Full" mode and save the settings.
    ...
    Config PPTP_L2TP    web    pptp    ${DEVICES.cisco.gateway}    ${ppp_username}    ${ppp_password}    ${PPTP_TYPE_SMART}    ${PPTP_START_WHEN_BOOT}    ${PPTP_FORCE_ENCRYPTION}
...
From Console, Check the PPTP status. Check traffic from lan to vpn internal lan
    [Arguments]
    [Documentation]    Test Step  
    # Get pptp ip so we can check if wanhost can received packet from this pptp address
    ${dut_wan_ip} =   Wait Until Keyword Succeeds    5x    5s     Get IP Value From Device SSH Connection    dut1_ssh     ${DEVICES.rf1.pptp_interface}
...
Restore WAN Setting    
    ...
    Config PPTP_L2TP    web    pptp    ${DEVICES.cisco.gateway}    ${ppp_username}    ${ppp_password}    full    False    False
