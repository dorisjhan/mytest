...
class CCFGGuiApiClass(WebSession):
    ...
    def dld_cfg_file(self, ccfg_ip, cfg_file_page, wf_name_val, wf_desc_val, ont_type, wf_oper_val, cfg_file, wf_type, wf_exec_wndw_val):
        ...
        try:
            ...
            if file_delete == True:
                print("Deleted Workflow: " + wf_name_val)
            else:
                print("Workflow: " + wf_name_val + " Not Found!")
            self.driver.get('http://' + ccfg_ip + '/' + cfg_file_page)
            #----------------------------------------------------------------------------------------------
            #  Start New Workflow by clicking New Workflow Button
            self.__click_element(how="id", what="btn-newWorkflow")
            ...
            print("Workflow Name: " + wf_name_val)
            print("Workflow Desc: " + wf_desc_val)
            time.sleep(2)
    ...
    def del_cfg_file(self, ccfg_ip, wf_page, wf_name_val):
        ...
        WebDriverWait(self.driver, 30)\
            .until(lambda s: s.find_element_by_xpath('//button[@id="btn-newWorkflow"]'))
        cfg_del_btn = '//tr[descendant::td[contains(.,"' + wf_name_val + '")]]//i[@data-original-title="Delete"]'
        cfg_susp_btn = '//tr[descendant::td[contains(.,"' + wf_name_val + '")]]//i[@data-original-title="Suspend"]'
        chk_susp_btn = self.chk_element_exist(how="xpath", what=cfg_susp_btn)
        if chk_susp_btn == True:
            print("Found Workflow " + wf_name_val + " In-progress!")
            self.__click_element(how="xpath", what=cfg_susp_btn)
            del_confirm_btn = '//button[@class="btn btn-warning pull-right confirm"]'
            self.__click_element(how="xpath", what=del_confirm_btn)
        chk_del_btn = self.chk_element_exist(how="xpath", what=cfg_del_btn)
        if chk_del_btn == True:
            print("Deleting Workflow for: " + wf_name_val)
    ...
    def chk_dld_status(self, ccfg_ip, wf_page, wf_name_val):
        ...
        :return:
        """
        wf_page_refresh = 'http://' + ccfg_ip + '/' + wf_page
        complete = 0
        self.driver.get(wf_page_refresh)
        WebDriverWait(self.driver, 30)\
            .until(lambda s: s.find_element_by_xpath('//button[@id="btn-newWorkflow"]'))
        pass_wf_status = '//td[contains(text(),"' + wf_name_val + '")]/following-sibling::td[contains(text(),"Completed")]'
        fail_wf_status = '//td[contains(text(),"' + wf_name_val + '")]/following-sibling::td[contains(text(),"Fail")]'
        inpro_wf_status = '//td[contains(text(),"' + wf_name_val + '")]/following-sibling::td[contains(text(),"Progress")]'
        while complete <= 10:
            self.driver.get(wf_page_refresh)
            WebDriverWait(self.driver, 30)\
                .until(lambda s: s.find_element_by_xpath('//button[@id="btn-newWorkflow"]'))
            dld_status_pass = self.chk_element_exist(how="xpath", what=pass_wf_status)
            dld_status_fail = self.chk_element_exist(how="xpath", what=fail_wf_status)
            dld_status_inpro = self.chk_element_exist(how="xpath", what=inpro_wf_status)
            if dld_status_pass == True:
                return True
            elif dld_status_fail == True:
                return False
            elif dld_status_inpro == True:
                time.sleep(10)
                complete += 1
                print("Download check #: " + str(complete))
        if inpro_wf_status == True and complete >= 10:
            return False
        return False
